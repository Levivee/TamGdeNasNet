...
define config.name = _("TamGdeNasNet")
...
define gui.about = _p("""Осталось 5 дней до дня побега. Из-за закрытых ворот манила желанная свобода. За эти пять дней Главному герою необходимо найти союзника, который разделит с ним тяжелый путь в будущее. 
	Пока что они просто школьники, готовящиеся сдать экзамены. Но после выпуска, через эти 5 дней, они станут полноправными членами системы. Бежать или остаться? С кем же предпочтет сбежать Главный герой?""")
...
define build.name = "TamGdeNasNet"
...
define config.sample_sound = "gui/test.ogg"
...
define config.main_menu_music = "audio/mainmenu-theme.mp3"
...
define config.save_directory = "TamGdeNasNet-1602100959"
